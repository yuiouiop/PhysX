...
int renderGeometry(unsigned int VAO, unsigned int VBO, const PxGeometryHolder& h){
	switch(h.getType()){
...
void renderActors(float* verticesBox, unsigned int* indicesBox, Model &chair, Shader &ourShader, Camera &camera, Shader &sampleShader, unsigned int VAO, unsigned int VBO, unsigned int VAOBOX, unsigned int VBOBOX, unsigned int EBOBOX, PxRigidActor** actors, const PxU32 numActors, bool shadows, const PxVec3 & color)
{
	...
	for(PxU32 i=0;i<numActors;i++)
	{
		...
		for(PxU32 j=0;j<nbShapes;j++){
			//if(nbShapes == 6) printf("shape number : %d\n", j);
			...
			glm::mat4 model(*(MAT + 0), *(MAT + 1), *(MAT + 2), *(MAT + 3), 
							*(MAT + 4), *(MAT + 5), *(MAT + 6), *(MAT + 7), 
							*(MAT + 8), *(MAT + 9), *(MAT + 10), *(MAT + 11), 
							*(MAT + 12), *(MAT + 13), *(MAT + 14), *(MAT + 15));
